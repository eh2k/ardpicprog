...
#define PIN_MCLR        1      // 0: MCLR is VPP voltage, 1: Reset PIC
...
const char s_pic16f73[]  PROGMEM = "pic16f73";
...
struct deviceInfo const devices[] PROGMEM = {
  ...
  {s_pic16f73,   0x0601, 4096, 0x2000, 0x0, 8, 1, 0, 0, FLASH6, EEPROM},
...
void beginProgramCycle(unsigned long addr, bool isData)
{
  switch (isData ? dataFlashType : progFlashType) {
    ...
    case FLASH5:
      ...
      break;
    case FLASH6:
      sendSimpleCommand(0x8); //XXE
      delayMicroseconds(DELAY_TPROG5);
      sendSimpleCommand(0xE);
      break;
  }
}
...
bool writeWord(unsigned long addr, unsigned int word)
{
  ...
  if (addr >= dataStart && addr <= dataEnd) {
    ...
    readBack = word << 1; //sendReadCommand(CMD_READ_PROGRAM_MEMORY);
    readBack = (readBack >> 1) & 0x3FFF;
  }
